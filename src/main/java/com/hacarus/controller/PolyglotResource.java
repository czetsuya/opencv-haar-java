...
public class PolyglotResource {
    ...
    @Produces("image/png")
    public Response detectObject(@RequestBody(name = "data", description = "Multipart form that contains an image to be process", required = true, content = @Content(mediaType = APPLICATION_JSON, schema = @Schema(implementation = MultipartBody.class))) @Valid @MultipartForm MultipartBody data) throws IOException {
